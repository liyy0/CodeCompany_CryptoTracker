...
class CoinNameViewModel (
        ...
        private val ids: String? ): ViewModel() {
        ...
        fun changeLocal(currency:String, locale: String){
            viewModelScope.launch {
                coinRepos.getAllCoinList(currency, locale, ids).collectLatest { result ->
                    when(result) {
                        is Result.Error -> {
        ...
        init {
            viewModelScope.launch {
                ...
                coinRepos.getAllCoinList("usd", "en", ids).collectLatest { result ->
                    when(result) {
                        is Result.Error -> {
